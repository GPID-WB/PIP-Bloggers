...
-   Simplest way to handling conditions is to [ignore them]{style="color:red;"}!
...
[**Notice:**]{style="color:red;"} They are applied in order.
...
-   [Important]{style="color:red;"}: for ***each specific survey*** we should have ***only one record*** (or row/observation).

::: callout-warning
### Objective:

1.  Signal if there are duplicates of `PFW` per `country_code`,`surveyid_year` and `survey_acronym`.
2.  Log which are the duplicates
3.  And continue with the process using only unique values.
:::
