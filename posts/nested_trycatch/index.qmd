...
When running a pipeline or loop over many inputs — like datasets, files, or parameter combinations — it's critical to:
...
This concludes our full pipeline: from simple `tryCatch()` to nested steps, error metadata, structured logging, and full-loop resilience.
...
## Appendix: Advanced Error Chaining with `rlang::abort(parent = ...)`

In complex pipelines, it's useful to let low-level functions raise errors with detailed context, and then let higher-level functions catch and re-raise those errors while preserving the original. This is called **error chaining**.

The `{rlang}` package supports this through the `parent` argument in `rlang::abort()`. Here's how it works in practice.

### 1. Define a helper to walk the error chain

We’ll need a function to search through a chain of errors and return the first one that matches a class:
...
This inspects the error object `cnd`, checks whether it inherits from the given `class`, and if not, moves to `cnd$parent`.



### 2. Define a failing step with custom metadata

Here we simulate a low-level function (`baz()`) that fails with additional metadata:
...
### 3. A mid-level wrapper that catches and rethrows

Now we define `foo()` that wraps calls to other functions (`bar()`, `baz()`, `goo()`) and rethrows errors with `baz()` as the parent:
...
foo <- function(x) {
  tryCatch(
    {
      bar(x)
      baz(x)  # this will fail
      goo(x)
    },
    error = function(e) {
      rlang::abort(
        message = "foo() failed at one of its steps",
        class = "foo_error",
        parent = e
      )
    }
...
This preserves the original `baz_error` as the parent of the new `foo_error`.

### 4. Define a top-level logging function

This function catches the error from `foo()`, logs it, and uses `find_condition()` to extract details from the inner-most failure:

```{r}
log_failure <- function(e) {
  cli::cli_alert_danger("Top-level failure in {.strong foo()}")
  cli::cli_alert("Main message: {conditionMessage(e)}")
  
  if (!is.null(cnd <- find_condition(e, "baz_error"))) {
    cli::cli_alert("Root cause: {.strong {cnd$foo_step}} failed with input {.code {cnd$input_value}}")
  } else {
    cli::cli_alert("No known step-specific error was found in the chain.")
  }
}
```


### 5. Run the pipeline and catch the error
...
This chaining mechanism lets you:

- Attach rich, context-specific metadata where the error actually occurred.
- Catch and log those details later, without polluting every layer of your pipeline.
- Maintain clean separation of concerns: lower-level functions don't worry about global logging, and higher-level functions don't need to know the internals of every substep.

This pattern is powerful for building robust, testable, and debuggable pipelines — especially when you want detailed failure reporting without overcomplicating function interfaces.
