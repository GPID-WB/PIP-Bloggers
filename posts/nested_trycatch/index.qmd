...
safe_step <- function(expr, step_name) {
  tryCatch(
    expr = eval(expr),
    error = function(e) {
      stop(sprintf("Step '%s' failed: %s", step_name, conditionMessage(e)), 
           call. = FALSE)
    }
  )
}
...
foo <- function(x) {
  ...
  safe_step(quote(baz(x)), "baz()")
  safe_step(quote(goo(x)), "goo()")
...
tryCatch(
  ...
  error = function(e) {
    ...
    # Here you could log the step name extracted from the message
  }
)
```
...
```{r}
baz <- function(x) stop("Division by zero")
```

Then running `foo()` will trigger an error with a clear message:
