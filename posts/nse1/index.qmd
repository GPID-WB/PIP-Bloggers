...
description: "Understanding how R treats code as first-class data is essential for anyone working with logging, metaprogramming, or tidy evaluation. In this post, we demystify the core tools of Non-Standard Evaluation: quote(), substitute(), deparse(), and eval(). Through practical examples and careful explanation, you'll learn how to capture expressions, convert them to strings, evaluate them later, and understand when these tools behave differently — and why it matters."
author:
...
At first glance, `quote()` and `substitute()` may seem interchangeable — and in some cases, they behave the same. For example, when used **at the top level**, both will return the same unevaluated expression:
...
Now define a function that compares `quote()`, `substitute()`, and direct evaluation:

```{R}
inspect <- function(expr) {
  list(
    quoted = quote(expr),         # Always returns the symbol 'expr'
    substituted = substitute(expr), # Captures what was passed to 'expr'
    evaluated = eval(expr)        # Actually runs the code
  )
}

```

Now call the function with quoted objects:
...
**Conclusion**:\
- Use `quote()` when you want to **manually construct reusable expressions** in code. - Use `substitute()` when you want to **capture what someone passed to a function**.
...
Thanks for sharing the full context — this is excellent. Your blog post is already strong in structure and clarity, and you're setting it up beautifully for the next section. Here's what I suggest:
...
log_eval_wrong <- function(expr) {
  ...
  cat("Trying to print expr: ", deparse(expr), "\n")
  result <- eval(expr)
  result
}
```

Calling `log_eval_wrong(x + 1)` will raise an error:
...
So we use: `code <- substitute(expr)` to **freeze** the user input before evaluation, and then use `eval(code)` to run it deliberately later. This gives us full control over when and how the code is evaluated — and lets us capture and display exactly what the user typed.
...
This code first throws an error, then returns 11 — as expected. This happens because when `expr` reaches `eval()` it is already too late bvecause it has already been evaluated to in `deparse()`, so `eval()` simply returns `11`. In the second case, where `x` is removed from the Global environment, `expr` is evaluated in `deparse()` and fails.  
That’s why `substitute()` is essential in any function that wants to control **when** and **where** evaluation happens.
...
When you call `eval(expr)` without an `envir`, R evaluates the expression in the current environment:
...
Here, `x` is found in the global environment because that’s where `eval()` looks by default.
...
Here, `mpg` is treated as a column inside `mtcars`, not as a variable in the global environment. This is exactly how `dplyr::filter(mtcars, mpg > 25)` works internally.


### How to Use It in Your Own Code

You can add an optional `.env` argument to your own functions to control evaluation location:

```{R}
log_eval_env <- function(expr, .env = parent.frame()) {
  code <- substitute(expr)
  text <- deparse(code)
  ...
  message("Successfully evaluated in custom environment: ", text)
  invisible(result)
}
```
...
```{R}
my_env <- rlang::env(a = 42)
log_eval_env(a + 1, .env = my_env)
```

This lets you use **environment-aware logging**, evaluation, and even scoping systems — crucial when building packages that need to be robust in different execution contexts (e.g., R Markdown, Shiny, pipelines).

### Summary

- `eval(expr)` runs in the current environment.
- `eval(expr, envir = some_env)` lets you **choose where names get resolved**.
- This allows functions to:
  - Evaluate in data frames (`dplyr`)
  - Simulate delayed execution (`purrr::map`)
  - Create reproducible scoped environments (`rlang::env()`)
