...
::: {.callout-important title="quote() vs substitute(): A Subtle but Critical Difference"}

When writing NSE-aware functions, it's crucial to know the difference between `quote()` and `substitute()`:

- `quote(expr)` captures the code *exactly as written*, always.
- `substitute(arg)` captures what the *caller* passed to a function argument.
...
Outside of a function, both return the same thing:
...
But **inside a function**, only `substitute()` gives you access to the original expression the user typed.
