...
date: "02/13/2025"
categories: [Error Handing, Warnings]
---

## Intro
Exception handling is a powerful technique to make your R code more robust and maintainable. In this post, we demonstrate how to use `tryCatch()` alongside custom conditions using `cli::cli_abort()` and `rlang::abort()` to intercept, classify, and log specific types of errors. This allows us to decide whether to stop execution or recover gracefully.

The example comes from a data-cleaning context where we want to check for duplicate rows in a dataset. If duplicates are found, we can log the issue, optionally stop execution, or continue after resolving the problem.
