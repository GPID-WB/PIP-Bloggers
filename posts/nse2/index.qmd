...
### 7.4 Never Assume an Argument Is a Quosure

Some tidy eval tools require **quosures** (`enquo()`, `expr()`) but others work on raw calls or symbols.

Always test:

```{R}
maybe_enquo <- function(arg) {
  if (rlang::is_quosure(arg)) {
    return(arg)
  } else {
    return(rlang::enquo(arg))
  }
}
```

This makes your code compatible with both interactive calls and programmatic use.



### 7.6 Add Meaningful Defaults and Fail-Safes

You can provide a fallback expression, or skip evaluation when the input is `NULL`.

```{R}
with_default <- function(expr = NULL) {
  if (is.null(expr)) {
    expr <- rlang::expr(42)
  }
