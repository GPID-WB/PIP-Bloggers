...
var <- "hp"
threshold <- 200
expr <- substitute(v > t, list(v = as.name(var), t = threshold))
expr
eval(expr, envir = mtcars)
```
...
### 5.3 Using `{rlang}`: `sym()`, `parse_expr()`, and `env_get()`

Base R’s tools work, but `{rlang}` provides safer and more composable alternatives for programmatic evaluation.

#### `rlang::sym()` – Safer Symbol Construction
...
This is equivalent to `as.name()` but designed to compose well with other `{rlang}` functions.

#### `rlang::parse_expr()` – From String to Expression
...
`parse_expr()` is better than `parse(text = ...)` because it returns a single expression, not a list.
...
You can also **limit** the search to specific environments only.
...
filter_by_name(mtcars, "hp", 150)
```

* `rlang::sym()` turns `"hp"` into the symbol `hp`.
* `rlang::expr()` + `!!` unquotes the symbol and value.
* `eval_tidy()` safely evaluates inside the data mask.

This is a robust, tidy-eval-friendly way to do programmatic data manipulation.


### Summary

| Task                           | Base R              | `{rlang}` Alternative     |
| ------------------------------ | ------------------- | ------------------------- |
| Get variable by name           | `get("x", env)`     | `env_get(env, "x")`       |
| Make symbol from string        | `as.name("x")`      | `sym("x")`                |
| Parse string into expression   | `parse(text = ...)` | `parse_expr()`            |
| Build programmatic expressions | `substitute()`      | `expr()` + `!!` + `sym()` |
| Evaluate inside data and env   | `eval(expr, envir)` | `eval_tidy()`             |
